...
__kmpc_fork_call: function(loc, argc, microtask)
{
    ...
    var output = Module['read']('add2.js', false);
    var idlocs = [];
    ...
    request('POST','http://localhost:8080/initialize-job/123', {json:{'file':output,'heap':HEAP32.toString()}});
    for (l=0; l<10; l++)
    {
        ...
        var res = request('POST','http://localhost:8080/send-job/123', {json:{'args':argarr,'microtask':microtask, 'jobID':l}});
